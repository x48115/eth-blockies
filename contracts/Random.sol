...
library Random {
    function generateColorBytes(uint16[] memory random, uint8 idx)
        public
        view
        returns (uint8[3] memory)
    {
        ...
        (uint8 r, uint8 b, uint8 g) =
            Color.convertHslToRgb(hue, saturation, lightness);
        return [r, g, b];
    }

    function generateColorHexString(uint16[] memory randomData, uint8 idx)
        public
        view
        returns (string memory)
    {
        ...
        return Hex.generateHexStringFromBytes(colorBytesMerged);
    }

    function generateData(bytes memory seed)
        public
        view
